# CLAUDE.md — StudyEnglish

英語（日常会話表現）の勉強ログを、技術ブログ寄りで遊び心もある **Astro** 製サイトにまとめるリポジトリ。
...
英語の日常会話表現の学習を、一貫した粒度・体裁の記事にして公開する。**記事化の依頼が来たら必ず
...
| 学習マップ（/map）に記事を載せる | `src/roadmap.ts`（ドメインの slugs に追記） |

## 開発コマンド

```bash
npm run dev        # ローカルプレビュー http://localhost:4321/StudyEnglish/ (base付き)
